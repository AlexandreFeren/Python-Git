...
class Main:
    ...
    def __init__(self):
        ...
        self.commands = {}
        result = [y[len(PATH)+1:].split("\\") for x in os.walk(PATH) for y in glob(os.path.join(x[0], '*.py'))]
        ...
        for path in result: exec("import {0}.{1} as {1}".format(".".join(path[:-1]),path[-1][:-3]))
        for fn in result: exec("self.commands['{0}'] = {0}.{0}".format(fn[-1][:-3]))
