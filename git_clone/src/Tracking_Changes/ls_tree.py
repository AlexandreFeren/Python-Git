...
def get_tracked_files():
    ...
    exclude_root_dirs = [i[1:-1]
                         for i in exclude if i[0] == "/" and i[-1] == "/"]
        ...
        if full_path == root:
            [
                ...
                for d in [
                    d
                    for d in dirs
                    if any(fnmatch.fnmatch(d, ex) for ex in exclude_root_dirs)
                ]
            ]
            [
                files.remove(f)
                for f in [
                    f
                    for f in files
                    if any(fnmatch.fnmatch(f, ex) for ex in exclude_root_all)
                ]
            ]
        [
            dirs.remove(d)
            for d in [
                d for d in dirs if any(fnmatch.fnmatch(d, ex) for ex in exclude_dirs)
            ]
        ]
        [
            files.remove(f)
            for f in [
                f for f in files if any(fnmatch.fnmatch(f, ex) for ex in exclude_all)
            ]
        ]
